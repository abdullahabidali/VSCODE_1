...
import { ChatEditKind, IModifiedEntryTelemetryInfo, IModifiedTextFileEntry, ISnapshotEntry, ITextSnapshotEntry, ITextSnapshotEntryDTO, STORAGE_CONTENTS_FOLDER, WorkingSetEntryState } from '../../common/chatEditingService.js';
...
export class ChatEditingModifiedFileEntry extends Disposable implements IModifiedTextFileEntry {
	...
	async restoreFromSnapshot(snapshot: ISnapshotEntry): Promise<void> {
		if (!(snapshot instanceof TextSnapshotEntry)) {
			throw new Error('Unexpected snapshot type');
		}
